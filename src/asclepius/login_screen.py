...
class Login:
    ...
    def widgets(self, app):
        ...
        self.captcha_label = Label(
            app, text="Enter Captcha:", font=("Times New Roman", 14, "bold")
        ).place(relx=0.1, rely=0.52, anchor=NW)
        self.captcha_entry = Entry(app, textvariable=self.captcha_text).place(
            relx=0.1, rely=0.60, width=300, height=30, anchor=NW
        )
        ...
        # window title
        self.title = Label(app, text="Login Here!", font=("Arial", 14, "bold")).pack(
            pady=40, padx=50
        )
        ...
        ).place(relx=0.1, rely=0.20, anchor=NW)
        self.username_entry = Entry(app, textvariable=self.username).place(
            relx=0.1, rely=0.27, width=300, height=30, anchor=NW
        )

        # creating password label and entry
        self.pswrd_label = Label(
            app, text="Password:", font=("Times New Roman", 14, "bold")
        ).place(relx=0.1, rely=0.40, anchor=NW)
        self.pswrd_entry = Entry(app, textvariable=self.password, width=20).place(
            relx=0.1, rely=0.47, width=300, height=30, anchor=NW
        )
        ...
        self.submit = Button(
            ...
            bg="light blue",
            command=self.submit,
        ).place(relx=0.5, rely=0.70, width=150, height=40, anchor=CENTER)
